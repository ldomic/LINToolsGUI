...
// Get the Object by ID
   ...
   d3Context.selectAll('g.residue')
        ...
        function setHBonds(){
          ...
          for (bond of HBonds){
            if (String(selectedRes) === String(bond.bond[0])){
              ...
              rec_height = parseInt(rect.attr("height"))
              // Barplot stuff
              for (l=0;l < bond.percentage.length;l++){
              var color = d3.scaleOrdinal(d3.schemeCategory10);
              my_data = bond.hbondTime[l];
              ...
              var svg = tooltip.append("g")
                  ...
                  .style("z-index","20")
                  .attr("transform",
                        "translate(" + String(parseFloat(x_coord)+margin.left)+ "," + String(parseFloat(y_coord)+160.0+50*l)+ ")");
              rect.attr("height",function(d){return rec_height+50*(l+1)})
              console.log(color(0),l)
              svg.append("text")
              .style("text-anchor","middle")
              .style("font-size",28)
              .style("font-family","monospace")
              ...
              .attr("x",width+20)
              .attr("y",23)
              .text(String(parseInt(bond.percentage))+"%")
              svg.selectAll(".bar")
                    .data(my_data)
                  .enter().append("rect")
                    .attr("x", function(d,j) {return x(j/my_data.length * width/my_data.length); })
                    ...
                    .attr("y", function(d) { return y(d); })
                    .attr("height", function(d) { return height - y(d); })
                    .style("fill",function(){return color(l)});
          }}
        };
      };
      setHBonds();
    ...
    d3Context.selectAll('ellipse.molAtom').
      on("mouseover", function(d){
